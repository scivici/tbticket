...
export async function getProducts(_req: Request, res: Response): Promise<void> {
  const products = await queryAll<any>('SELECT * FROM products WHERE deleted_at IS NULL ORDER BY id');
  res.json(products);
}
...
export async function getCategories(req: Request, res: Response): Promise<void> {
  ...
  const categories = await queryAll<any>(
    'SELECT * FROM product_categories WHERE product_id = ? AND deleted_at IS NULL ORDER BY display_order',
    [productId]
  );
  res.json(categories);
}
