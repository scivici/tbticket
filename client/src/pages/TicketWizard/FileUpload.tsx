...
export default function FileUpload({ data, onUpdate, onNext, onPrev }: Props) {
  ...
  const addFiles = useCallback((newFiles: FileList | File[]) => {
    ...
    if (data.files.length + arr.length > MAX_FILES) {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mb-4">Upload screenshots, logs, or any files that help describe the issue. Max 10MB per file, up to 5 files.</p>
      ...
      >
        ...
        <label className="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 cursor-pointer">
          ...
          <input type="file" multiple className="hidden"
            accept="image/*,.pdf,.txt,.csv,.log,.json,.zip"
            onChange={e => e.target.files && addFiles(e.target.files)} />
        </label>
        <p className="text-xs text-gray-400 mt-2">PDF, images, text, log files, JSON, ZIP</p>
      </div>
        ...
        <div className="mt-4 space-y-2">
          {data.files.map((file, index) => (
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <button onClick={() => removeFile(index)} className="p-1 text-gray-400 hover:text-red-500">
                <X className="w-4 h-4" />
              </button>
            </div>
          ))}
