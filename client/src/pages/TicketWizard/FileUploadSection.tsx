...
import { Upload, X, FileText, Image, Info, Clipboard } from 'lucide-react';
...
export default function FileUploadSection({ files, onChange, productName = '' }: Props) {
  ...
  return (
    <div>
      <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-1">Attach Files <span className="text-sm font-normal text-gray-500">(optional)</span></h3>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">Upload screenshots, logs, or SIP traces. Max 200MB per file, up to 10 files.</p>
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm text-blue-700 dark:text-blue-300">
            ...
            <ul className="list-disc ml-4 space-y-1 text-xs">
              ...
              {!p.includes('tmg') && !p.includes('tsg') && !p.includes('prosbc') && (
                <>
      ...
      <div onDragOver={e => { e.preventDefault(); setDragActive(true); }} onDragLeave={() => setDragActive(false)} onDrop={handleDrop}
        ...
        {pasteFlash ? (
          <>
            <Clipboard className="w-8 h-8 text-green-500 mx-auto mb-2" />
            <p className="text-green-600 dark:text-green-400 font-medium">Screenshot pasted!</p>
          </>
        ) : (
          <>
            <Upload className="w-8 h-8 text-gray-500 mx-auto mb-2" />
            <p className="text-gray-500 dark:text-gray-400 mb-2 text-sm">Drag and drop files here, or</p>
