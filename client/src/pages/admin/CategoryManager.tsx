...
export default function CategoryManager() {
  ...
  return (
    <div>
      ...
      <div className="mb-6"><label className="block text-sm font-medium text-gray-300 mb-1">Select Product</label>
      ...
      {(creating || editing) && (
        <div className="tb-card border-primary-500/30 p-6 mb-6">
          <h3 className="font-semibold text-white mb-4">{creating ? 'New Category' : 'Edit Category'}</h3>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div><label className="block text-sm font-medium text-gray-300 mb-1">Icon</label><select value={form.icon} onChange={e => setForm(f => ({ ...f, icon: e.target.value }))} className="tb-select w-full"><option value="">None</option>{ICONS.map(i => <option key={i} value={i}>{i}</option>)}</select></div>
          ...
          <div key={c.id} className="tb-card p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex flex-col gap-0.5">
                <button onClick={() => moveCategory(index, -1)} disabled={index === 0} className="p-0.5 text-gray-600 hover:text-gray-300 disabled:opacity-30"><ChevronUp className="w-4 h-4" /></button>
                <button onClick={() => moveCategory(index, 1)} disabled={index === categories.length - 1} className="p-0.5 text-gray-600 hover:text-gray-300 disabled:opacity-30"><ChevronDown className="w-4 h-4" /></button>
              </div>
              ...
              <div><h3 className="font-medium text-white">{c.name}</h3><p className="text-sm text-gray-500">{c.description}</p></div>
              {c.icon && <span className="px-2 py-0.5 bg-gray-700 text-gray-400 text-xs rounded">{c.icon}</span>}
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => startEdit(c)} className="p-2 text-gray-400 hover:text-accent-blue hover:bg-white/10 rounded-lg"><Pencil className="w-4 h-4" /></button>
              <button onClick={() => handleDelete(c.id, c.name)} className="p-2 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
