        ...
        {engineers.map(eng => (
          <div key={eng.id} className="tb-card overflow-hidden">
            ...
            {expanded === eng.id && detail && (
              <div className="border-t border-gray-700 p-5 bg-tb-bg">
                <div className="grid md:grid-cols-2 gap-8">
                  {/* Skills */}
                  <div>
                    <h4 className="font-medium text-sm text-white mb-3">Skills</h4>
                              ...
                              <div className="flex gap-0.5">
                                {[1, 2, 3, 4, 5].map(l => (
                                  <button key={l} onClick={() => updateSkill(eng.id, skill.id, l)} className="p-0.5 hover:scale-110 transition-transform">
                                    <Star className={`w-4 h-4 ${l <= prof ? 'fill-yellow-400 text-yellow-400' : 'text-gray-600 hover:text-gray-400'}`} />
                                  </button>
                                ))}
                              </div>
                              {prof > 0 && (
                                <button onClick={() => removeSkill(eng.id, skill.id)} className="p-0.5 text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity ml-1">
                                  <X className="w-3 h-3" />
                                </button>
                              )}
                            </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <h4 className="font-medium text-sm text-white">Product Expertise</h4>
                    ...
                    <div className="space-y-2">
                      {(detail.expertise || []).map((exp: any, i: number) => (
                        <div key={i} className="flex items-center justify-between group">
                          <span className="text-sm text-gray-300 truncate max-w-[180px]" title={`${exp.productName} / ${exp.categoryName || 'General'}`}>
                            ...
                            <div className="flex gap-0.5">
                              {[1, 2, 3, 4, 5].map(l => (
                                <button key={l} onClick={() => updateExpertiseLevel(eng.id, i, l)} className="p-0.5 hover:scale-110 transition-transform">
                                  <Star className={`w-4 h-4 ${l <= exp.expertiseLevel ? 'fill-accent-blue text-accent-blue' : 'text-gray-600 hover:text-gray-400'}`} />
                                </button>
                              ))}
                            </div>
                            <button onClick={() => removeExpertise(eng.id, i)} className="p-0.5 text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity ml-1">
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                        </div>
                    ...
                    {/* Add expertise form */}
                    {addingExpertise && (
                      <div className="mt-3 p-3 bg-tb-card rounded-lg border border-gray-700 space-y-3">
                        <div>
                        ...
                        <div>
                          <label className="block text-xs text-gray-400 mb-1">Expertise Level</label>
                          <div className="flex gap-1">
                            {[1, 2, 3, 4, 5].map(l => (
                              <button key={l} onClick={() => setExpLevel(l)} className="p-0.5">
                                <Star className={`w-5 h-5 ${l <= expLevel ? 'fill-accent-blue text-accent-blue' : 'text-gray-600'}`} />
                              </button>
                            ))}
                          </div>
                        </div>
